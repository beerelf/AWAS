		...
		for (int iyear=0; iyear<imp_nyears; iyear++) {
			for (int imonth=0; imonth<nmonths; imonth++) {
				...
				if (!compactFormat || imonth > 0) {
					// Compact form does not read each line here but 
					istr.getline(buf, sizeof(buf));
				}
				istringstream istrstr(buf);
				...
				if (yearToken != GetStartYear() + iyear && !compactFormat) {
					AfxMessageBox(("Read pump year " + to_string<int>(yearToken)
						+ " for well " + sd.m_header1
						+ "; should be year "
				...
				if (m_timeUnits == SiteData::DAYS) {
					...
					ntokens = getDaysInMonth(imonth, yearToken, (ItemData::YearTypeEnum)m_yearMode);
				}
				...
				while (istrstr.good() && cnt < ntokens) {
					...
					sd.m_pumpingRecTD.SetValueDate((ItemData::YearTypeEnum)m_yearMode, PumpingRecTD::Delta, 1, yearToken, month_date, day_date);
					sd.m_pumpingRecTD.SetValueDate((ItemData::YearTypeEnum)m_yearMode, PumpingRecTD::Q, gpmval, yearToken, month_date, day_date);
					sd.m_metaDataTD.SetValueDate((ItemData::YearTypeEnum)m_yearMode, MetaDataTD::MetaData, gpmval, yearToken, month_date, day_date);
...
{
	// Check if we need to zero out data at the beginning because the sim start month is non-zero.
...
SitesManager::Execute()
{
	...
	for (l = m_siteList.begin(); l != m_siteList.end(); ++l) {
		...
		if (m_useIgnoreYear && m_displayMode == DISPLAY_MODIFIED) {
			// Zero out pumping if ignore year is set.
			l->m_pumpingRecTD.Fill(PumpingRecTD::Q, 0, m_runIgnoreYear+1, m_simEndYear);
		}
	}
		...
		if (m_displayMode == DISPLAY_MODIFIED)  {
			if (m_timeUnits == SiteData::MONTHS) {
				...
				if (!useAverageDaysInMonth) {
					// Now we need to apply a conversion from actual days in the month to average days.
					int startYear = l->m_pumpingRecTD[PumpingRecTD::Q].m_startyear;
					int nyears = l->m_pumpingRecTD.GetNYears();
					for (int iyear=0; iyear<nyears; iyear++) {
						for (int imonth=0; imonth<12; imonth++) {
							double ndays = getDaysInMonth(imonth, iyear + startYear);
							double val = l->m_pumpingRecTD.GetValueDate(ItemData::CALENDAR, ItemData::MONTHLY, PumpingRecTD::Q, iyear, imonth);
							if (val != SitesManager::NoData)
							{
								val *= 30.41667 / ndays;
							}
							l->m_pumpingRecTD.SetValueDate(ItemData::CALENDAR, PumpingRecTD::Q, val, iyear, imonth);
						}      
					}
